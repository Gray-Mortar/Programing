(function () {
  ...
  function renderVotes(record) {
    ...
    classes.forEach(function (label) {
      ...
      if (bar)
        bar.style.width = record.k ? (count / record.k) * 100 + "%" : "0%";
  ...
  function renderNeighbors(record) {
    ...
    if (!neighbors.length) {
      body.innerHTML =
        '<tr><td colspan="7">这次记录没有保存邻居明细。</td></tr>';
      return;
    }
    body.innerHTML = neighbors
      .map(function (neighbor, index) {
  ...
  function render(record) {
    ...
    document.getElementById("record-summary").textContent =
      ...
      record.k +
      " 的最近邻投票。";
    document.getElementById("record-predicted").textContent =
      record.predicted || "-";
    document.getElementById("record-confidence").textContent =
      record.confidence !== undefined ? record.confidence + "%" : "-";
    document.getElementById("record-k").textContent = record.k || "-";
    document.getElementById("record-time").textContent = formatTime(
      record.time,
    );
    document.getElementById("record-features").textContent =
      "特征向量：[" + (record.features || []).join(", ") + "]";
    renderVotes(record);
