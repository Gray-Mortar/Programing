...
function resetResults() {
    ...
    document.querySelectorAll(".probability-bar").forEach((bar) => { bar.style.width = "0"; });
    document.querySelectorAll(".probability-value").forEach((value) => { value.textContent = "0.0%"; });
    previewContext.fillStyle = "#000";
...
function clearCanvas() {
    ...
    resetResults();
    message.textContent = session ? "请在画板中央写一个数字。" : "请等待模型加载完成。";
}
...
function preprocessCanvas() {
    const source = drawingContext.getImageData(0, 0, drawingCanvas.width, drawingCanvas.height);
...
function showProbabilities(probabilities) {
    probabilities.forEach((probability, digit) => {
        ...
        document.querySelector(`[data-value="${digit}"]`).textContent = `${percent.toFixed(1)}%`;
    });
}
...
async function loadModel() {
    try {
        ...
        session = await ort.InferenceSession.create("model/mnist-8.onnx", { executionProviders: ["wasm"] });
